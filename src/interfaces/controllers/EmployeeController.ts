...
export class EmployeeController {
  static async registerEmployee(req: Request, res: Response) {
    try {
      ...
      console.log("Registering employee with data:", {
        name,
        email,
        password,
        divisionId,
      ...
      if (!supervisorId || !companyId) {
        return sendResponse(
          res,
          403,
          "Forbidden: Supervisor or Company ID not found"
        );
      }
  ...
  static async updateEmployee(req: Request, res: Response): Promise<void> {
    try {
      ...
      const updateUseCase = new UpdateEmployeeUseCase(
        employeeRepository,
        userRepository
      );
  ...
  static async getProfile(req: Request, res: Response) {
    try {
      const id = res.locals.positionId;
      const useCase = new GetEmployeeUseCase(employeeRepository);
      const result = await useCase.execute(id);
